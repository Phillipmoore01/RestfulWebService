...
{
	$sql="Insert into `Tech` (`device_type`,`manufacturer`,`serial_number`) values ('$row[0]','$row[1]','$row[2]')";
	if($row[2] == ''){
		echo("Skipping line $count of the data set\n");
		// add to log 
	}
	elseif(preg_match('/\'/',$row[1])){
		preg_replace("/[^A-Za-z0-9]/",'',$row[1]);
		// add to log
	}
	elseif(preg_match('/\'/',$row[0])){
		preg_replace("/[^A-Za-z0-9]/",'',$row[0]);
		// add to log
	}
	else
