...
<?php
    if (isset($_POST['submit']))
    {
		...
		$sql="Select `serial_number` from `Tech` where `serial_number`='$serialNumber'";
        $rst=$dblink->query($sql) or die("<p>Something went wrong with $sql<br>".$dblink->error);
        if ($rst->num_rows<=0)//sn not previously found
        {
            $sql="INSERT INTO `Tech` (`device_type`, `manufacturer`, `serial_number`, `auto_id`) values ('$device','$manufacturer','$serialNumber','0')";
            $dblink->query($sql) or die("<p>Something went wrong with $sql<br>".$dblink->error);
            redirect("web/index.php?msg=EquipmentAdded");
        }
        else
            redirect("web/add.php?msg=DeviceExists");
    }
?>